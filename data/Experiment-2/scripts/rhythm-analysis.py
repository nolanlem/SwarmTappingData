...
#%% take all-stim-tap-stats.csv as generated from ITI-subjects-all-beats.py and add columns with 
## |R|, center bpm, per stimuli 
#batch = 'all'

#%% NB: just sanity check... testing before loop w single stim
# stim = 'strong_72_1'
# paramdir = './stim-no-timbre-5/stimuli_1/'


# bwindir= os.path.join(paramdir, 'phases', 'beat-windows', stim + '.txt')
# bwindows = np.loadtxt(bwindir)
# bwindows = s2t(bwindows)

# p = mapZCs2Circle(zcs, bwindows)
# r, ps = getCircMeanVector(p)
# print(r,ps)

# y, sr_ = librosa.load('./allstims/strong_n_72_1.wav')
# plt.vlines(t2s(bwindows), -1.5, 1.5)
# plt.plot(y)
# for osc in zcs:
#     plt.vlines(t2s(osc)[:19], -2.5, -1, color='red', linewidth=0.5)
# ax = plt.gca()
# ax.set_xticks(np.arange(0, len(y), 22050*2))
# ax.set_xticklabels([elem/(22050) for elem in ax.get_xticks()])
...
for i, stim in enumerate(unique_stimuli):
    ...
    for stimdir, stimint in zip(stimdirs, ['1','2','3','4']):
        if stimsplit[-1] == stimint:
            rpath = os.path.join(stimtimbredir, stimdir, 'phases', 'pc', bname)
            cbpmpath = os.path.join(stimtimbredir, stimdir, 'phases','center-bpm', bname)
            bwindir = os.path.join(stimtimbredir, stimdir, 'phases','beat-windows', bname)
            trigsdir = os.path.join(stimtimbredir, stimdir, 'trigs', bname.split('.')[0] + '.npy')
...
for i in range(df.shape[0]):
    subj = df.iloc[i, 0]    # the subject name in .csv format
    thestim = df.iloc[i, 1]     # the stimulus
...
Rmodel = df['R model'].values
normiti = df['norm iti subj'].values
...
'bb_kura-A2_2020-08-08_23h32.56.541.csv',
'PARTICIPANT_kura-A2_2020-08-28_09h24.01.653.csv',
'cc_kura-B1_2020-08-09_00h01.21.424.csv',
...
def frmsd_iso(taps, shift_res=60, freq_res=30, freqlow=0.5, freqhigh=7):  
    ...
    shifts = np.linspace(0, 1/freqlow, shift_res)
    ...
    for i,freq in enumerate(freqshifts):
        ...
        shiftspan = shifts[shifts <= bbins[1]]
...
df = pd.read_csv('./mturk-csv/usable-batch-12-7/subject-raster-plots/nt/all/k-stats-4-7-nt.csv')

subjbatch = [unique_subjects[0]]

f = open(plotdir + 'histograms/csvs/kde-comparison.csv', 'w')
writer = csv.writer(f)
writer.writerow(['subject', 'stim', 'bc iti', 'taps iti', 'bc iti histo', 'tap iti histo'])



for n,person in enumerate(subjbatch):
    ...
    for i, sc in enumerate(stimbatch):
        fig, ax = plt.subplots(nrows=len(sc), ncols=5, figsize=(30,16), sharey='col')
        fig1, ax1 = plt.subplots(nrows=len(sc), ncols=1, figsize=(10,10))
        ...
        for j, stim in enumerate(sc):
            ...
            taps = df[(df['subject'] == person) & (df['stim'] == stim)]['taps'].apply(literal_eval)
            taps = list(taps)[0]
            ...
            bins=np.histogram(np.hstack((tapiti,bciti)), bins=50)[1] #get the bin edges



                       
            kdetaps= sns.distplot(tapiti, hist=True, bins=bins, ax=ax[j,1], kde=True, color='red', label='taps')
            kdebcs = sns.distplot(bciti, hist=True, bins=bins, ax=ax[j,1], kde=True, color='blue', label='stim')
            
            for line, c in zip(ax[j,1].lines, ['red', 'blue']):
                thekde_x = line.get_xdata()
                thekde_y = line.get_ydata()
                
                ax1[j].plot(thekde_x, thekde_y, color=c)
            ax1[j].set_title(stim)
            
            # kde_taps = kdetaps.get_lines()[0].get_data()[1]
            # kde_bcs = kdebcs.get_lines()[0].get_data()[1]
            
            # writer.writerow([person, stim, bciti, tapiti, kde_bcs, kde_taps])            

            # ax1[j].plot(kde_bcs, color='blue')
            # ax1[j].plot(kde_taps, color='red')            

        
                        
            #### GAT Pulse Analysis ####
            frmsds_t, freqbins_t = frmsd_iso(taps, shift_res=30, freq_res=20)
            frmsds_bc, freqbins_bc = frmsd_iso(s2t(bcenters), shift_res=30, freq_res=20) 

            # invert 
            i_tapgat = np.array(frmsds_t)*-1 
            i_bcgat = np.array(frmsds_bc)*-1                        
            ## PLOT GAT
